...
// Upload a large blob directly to the Gemini File API, bypassing Vercel's 4.5 MB body limit.
// Returns the file URI to use in generateContent.
async function uploadBlobToFileApi(blob) {
  const mimeType = blob.type || 'audio/webm';

  // Step 1: get a resumable upload session URL from our server (keeps API key server-side)
  ...
  // Step 2: upload blob directly to Google (no Vercel size limit involved)
  ...
  if (!uploadRes.ok) throw new Error('Audio upload to Gemini failed');
